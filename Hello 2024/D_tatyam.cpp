...
/*
 差分が 1 であるところを選び、大きい方を消す操作 => 上から貪欲？
 */
bool solve(){
    LL(n);
    VEC(ll,a,n);
    if(count(a,0)!=1)return 0;
    vec(ll,l,n);
    vec(ll,r,n);
    rep(n){
        l[i]=i-1;
    ...
    vec(vector<ll>,idx,n);
    rep(n)idx[a[i]].push_back(i);
    rrep(x,1,n){
        auto&v=idx[x];
        function<void(ll)>remove=[&](ll i){
            if(i<0||i>=n)return;
            if(a[i]!=x)return;
            if(l[i]==-1&&r[i]==-1)return;
            const ll L=l[i],R=r[i];
            if(0<=L&&a[L]==a[i]-1||R<n&&a[R]==a[i]-1){
                if(0<=L)r[L]=R;
                if(R<n)l[R]=L;
                l[i]=-1;
                r[i]=-1;
        ...
        each(i,v)remove(i);
        each(i,v)if(l[i]!=-1||r[i]!=-1)return 0;
    }
    return 1;
}
int main(){
