...
struct centroid {
  ...
  void app(int u, int id, int l, int len, int last) {
    ans[u] = l;
    if(len==0) return;
    // debug() <<"before " <<imie(u) <<imie(id) <<imie(l) <<imie(len) <<imie(last);
    // debug() <<imie(lazy[u]);
    auto it = lazy[u].lower_bound({{len,-1},{-1,-1}});
    vector<pipi> toBeErased;
    vector<pipi> toBeAdded;
    if(it!=lazy[u].end()) {
        if(it->se.fi<id) {
            if(it->fi.fi==len) {
                toBeErased.push_back(*it);
            }
            toBeAdded.push_back({{len,l},{id,last}});
        }
    } else {
        toBeAdded.push_back({{len,l},{id,last}});
    }
    while(it!=lazy[u].begin()) {
        it--;
        if(it->se.fi<id) {
            toBeErased.push_back(*it);
        } else {
  ...
  void update(int v, int l, int r) {
    ...
    void appLazy(int u) {
        // debug() <<imie(u);
        if(sz(lazy[u])==0) return;
        for(auto it=--(lazy[u].end());;it--) {
            auto [lenl, idlast] = *it;
            auto [len, l] = lenl;
            auto [id,last] = idlast;
            len--, l++;
