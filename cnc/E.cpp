...
struct hld {
  ...
  inline seg_t seg_lazy_apply(seg_t lazy_val, seg_t new_val) {
    return max(new_val,lazy_val);
  }

  inline seg_t seg_lazy_func(seg_t cur_val, seg_t lazy_val, int l, int r) {
    return max(lazy_val,cur_val);
  }

  const seg_t seg_sentinel = -1e9;
  const seg_t seg_lazy_sentinel = -1e9;
  const seg_t seg_init_val = -1e9;
  /* END SECTION */
  ...
  seg_t query(int u, int v) {
    int lc = lca(u, v);
    seg_t qc1 = query_chain(u, lc), qc2 = query_chain(v, lc);
    cout <<qc1 <<" " <<qc2 <<"\n";
    seg_t val = seg_combine(qc1,qc2);
    seg_t temp = seg_query_header(label[lc],label[lc]);
    cout <<val <<" " <<temp <<"\n";
    return seg_combine(val, temp);
  }
