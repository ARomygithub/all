...
void solve() {
    ...
    for(int i=0;i<sz(vm);) {
        ...
        int cek = r;
        int rm = (r-1+sz(hull))%sz(hull);
        int rp = (r+1)%(sz(hull));
        ll valr = 1ll*hull[r].x*a+1ll*hull[r].y*b;
        while(rp!=cek && !(valr> 1ll*hull[rm].x*a+1ll*hull[rm].y*b && valr>= 1ll*hull[rp].x*a+1ll*hull[rp].y*b)) {
            rm = r;
            r = rp;
            l = rp;
            rp = (r+1)%sz(hull);
            valr = 1ll*hull[r].x*a+1ll*hull[r].y*b;
        }
        l = r;
        if(rp==cek) {
            sall += j-i;
