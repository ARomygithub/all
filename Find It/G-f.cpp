...
int main() { 
    ...
    set<PT> l1_l2 = seg_seg_intersection_inside(a1, a2, b1, b2);
    if (l1_l2.size() == 0 || l1_l2.size() == 2) {
        cout << "negative" << '\n';
    } else{
        PT tipot_l1_l2 = *l1_l2.begin();
        double panjang_l1 = dist(a1, a2);
        double panjang_l2 = dist(b1, b2);

        double selisih_l1_l2 = abs(panjang_l1 - panjang_l2);
        if(sign(2*selisih_l1_l2 - panjang_l1) <= 0 && sign(2*selisih_l1_l2 - panjang_l2) <= 0){
            set<PT> l1_l3 = seg_seg_intersection_inside(a1, a2, c1, c2);
            if(l1_l3.size() == 0 || l1_l3.size() == 2){
                cout << "negative\n";
            } else {
                set<PT> l2_l3 = seg_seg_intersection_inside(b1, b2, c1, c2);
                if(l2_l3.size() ==0 || l2_l3.size()==2){
                    cout << "negative\n";
                } else {
                    PT titik_l1_l3 = *l1_l3.begin();
                    PT titik_l2_l3 = *l2_l3.begin();
                    double panjang_v2_v1 = dist(titik_l1_l3, tipot_l1_l2);
                    double panjang_v3_v1 = dist(titik_l2_l3, tipot_l1_l2);
                    if(sign(4*panjang_v2_v1-panjang_l1) <= 0 && sign(4*panjang_v3_v1-panjang_l2) <= 0){
                        cout << "negative\n";
                    } else {
                        double sudut_dalam_rad = get_angle(titik_l1_l3-tipot_l1_l2, titik_l2_l3-tipot_l1_l2);
                        if(sign(sudut_dalam_rad - PI/2) <= 0){
                            double komplemen_sudut_v2 = get_angle(tipot_l1_l2-titik_l1_l3, titik_l2_l3-titik_l1_l3);
                            if(sign(komplemen_sudut_v2 - PI/2) <= 0){
                                double komplemen_sudut_v3 = get_angle(tipot_l1_l2-titik_l2_l3, titik_l1_l3-titik_l2_l3);
                                if(sign(komplemen_sudut_v3 - PI/2) <= 0){
                                    int orientations = orientation(tipot_l1_l2, titik_l1_l3, titik_l2_l3);
                                    if(orientations != -1){
                                        cout << "negative\n";
                                    } else {
                                        cout << "positive\n";
